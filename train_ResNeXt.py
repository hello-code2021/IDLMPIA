...
from network.ClassicNetwork.ResNeXt import ResNeXt50
...
def main():
    ...
    best_acc = 0.00
    net = ResNeXt50(num_classes=CLASS_NUMBER).to(device)
    ...
    f_loss = open(os.path.join(opt.train_result, 'ResNeXt50', 'train_loss.txt'), 'a')
    f_acc = open(os.path.join(opt.train_result, 'ResNeXt50', 'train_acc.txt'), 'a')

    print("Start Training ResNeXt50")
    ...
    for epoch in range(pre_epoch, EPOCH):
        ...
        if (epoch+1) % 1 == 0: # start from 0.
            ...
            f_acc1 = open(os.path.join(opt.train_result, 'ResNeXt50', 'test_acc.txt'), 'a')
            f_loss1 = open(os.path.join(opt.train_result, 'ResNeXt50', 'test_loss.txt'), 'a')
            with torch.no_grad():
                correct = 0
                total = 0
                labels_list = []
                ...
                if acc >= best_acc:
                    ...
                    torch.save(net.state_dict(), os.path.join(opt.train_result, 'ResNeXt50', 'net_best.pth'))
    ...
    torch.save(net.state_dict(), os.path.join(opt.train_result, 'ResNeXt50', 'net_latest.pth'))
