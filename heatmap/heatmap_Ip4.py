    ...
    def __call__(self, x):
        ...
        for name, module in self.model._modules.items():
            ...
            if name in self.target_layers:
                ...
                for nm1, mod1 in module._modules.items():
                    x = mod1(x)
                    x.register_hook(self.save_gradient)
                    target_activations += [x]
                # x = module(x)
...
def get_args():
    ...
    parser.add_argument('--weight-path', type=str, default='./model/InceptionV4_1209/net.pkl',
                        help='pretrained weight path')
    parser.add_argument('--image-path', type=str, default='./data/para_test',
                        help='Input image path')
    parser.add_argument('--output-path', type=str, default='./results/0117test/Ip4',
                        help='Input image path')
