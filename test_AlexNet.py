...
def main():
    ...
    print("using {} images for test.".format(len(test_data)))
    ...
    model.load_state_dict(torch.load(os.path.join(opt.model_weight_path,'AlexNet', 'net_best.pth'), map_location=device))
    model.eval()

    print('start to test')
    f_acc1 = open(os.path.join(opt.test_result_dir, 'AlexNet', 'test_acc.txt'), 'a')
    ...
    with torch.no_grad():
        ...
        save_file(os.path.join(opt.test_result_dir, 'AlexNet', 'con_predicted.txt'), predited_list)
        save_file(os.path.join(opt.test_result_dir, 'AlexNet', 'con_labels.txt'), labels_list)
