...
def main():
    ...
    model.load_state_dict(torch.load(os.path.join(opt.model_weight_path,'ResNet50', 'net_best.pth'), map_location=device))

    model.eval()

    print('start to test')
    f_acc1 = open(os.path.join(opt.test_result_dir, 'ResNet50', 'test_acc.txt'), 'a')
    ...
    with torch.no_grad():
        ...
        for data in test_loader:
            ...
            for i in predicted:
                predited_list.append( str(":") + str(i.item()) + '\n')
            for i in labels.data:
                labels_list.append( str(":") + str(i.item()) + '\n')
        ...
        save_file(os.path.join(opt.test_result_dir, 'ResNet50', 'con_predicted.txt'), predited_list)
        save_file(os.path.join(opt.test_result_dir, 'ResNet50', 'con_labels.txt'), labels_list)
