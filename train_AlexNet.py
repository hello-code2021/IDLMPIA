...
def main():
    ...
    best_acc = 0.00
    net = AlexNet(num_classes=CLASS_NUMBER).to(device)
    ...
    f_loss = open(os.path.join(opt.train_result, 'AlexNet', 'train_loss.txt'), 'a')
    f_acc = open(os.path.join(opt.train_result, 'AlexNet', 'train_acc.txt'), 'a')
    ...
    for epoch in range(pre_epoch, EPOCH):
        ...
        if (epoch+1) % 1 == 0: # start from 0.
            ...
            f_acc1 = open(os.path.join(opt.train_result, 'AlexNet', 'test_acc.txt'), 'a')
            f_loss1 = open(os.path.join(opt.train_result, 'AlexNet', 'test_loss.txt'), 'a')
            with torch.no_grad():
                correct = 0
                total = 0
                labels_list = []
                ...
                if acc >= best_acc:
                    ...
                    torch.save(net.state_dict(), os.path.join(opt.train_result, 'AlexNet', 'net_best.pth'))
                else:
