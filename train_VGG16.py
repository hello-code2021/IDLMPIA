...
def main():
    ...
    best_acc = 0.00
    net = VGG16(num_classes=CLASS_NUMBER).to(device)
    ...
    f_loss = open(os.path.join(opt.train_result, 'VGG16', 'train_loss.txt'), 'a')
    f_acc = open(os.path.join(opt.train_result, 'VGG16', 'train_acc.txt'), 'a')
    ...
    for epoch in range(pre_epoch, EPOCH):
        ...
        # validation
        if (epoch+1) % 1 == 0: # start from 0.
            print('start to validation')
            f_acc1 = open(os.path.join(opt.train_result, 'VGG16', 'test_acc.txt'), 'a')
            f_loss1 = open(os.path.join(opt.train_result, 'VGG16', 'test_loss.txt'), 'a')
            with torch.no_grad():
                correct = 0
                total = 0
                labels_list = []
                ...
                print('test accuracy:{}%, time:{}'.format(round(acc.item(), 3), time.time() - since))

                if acc >= best_acc:
                    best_acc = acc
                    print("upgrade best_acc:{}, save model".format(best_acc))
                    torch.save(net.state_dict(), os.path.join(opt.train_result, 'VGG16', 'net_best.pth'))
                else:
                    print("the best_acc was:{},no upgrade best_acc, no save model".format(best_acc))
                if acc>= 99:
                    torch.save(net.state_dict(), os.path.join(opt.train_result, 'VGG16', 'net_latest.pth'))
                    return
