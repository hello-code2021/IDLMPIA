...
def main():
    ...
    # load model weights
    model.load_state_dict(torch.load(os.path.join(opt.model_weight_path,'VGG16', 'net_best.pth'), map_location=device))
    model.eval() # closs dropput.

    print('start to test')
    f_acc1 = open(os.path.join(opt.test_result_dir, 'VGG16', 'test_acc.txt'), 'a')
    ...
    with torch.no_grad():
        ...
        # save the file.
        save_file(os.path.join(opt.test_result_dir, 'VGG16', 'con_predicted.txt'), predited_list)
        save_file(os.path.join(opt.test_result_dir, 'VGG16', 'con_labels.txt'), labels_list)
