...
def main():
    ...
    best_acc = 0.00
    net = DenseNet121(num_classes=CLASS_NUMBER).to(device)
    ...
    f_loss = open(os.path.join(opt.train_result, 'DenseNet121', 'train_loss.txt'), 'a')
    f_acc = open(os.path.join(opt.train_result, 'DenseNet121', 'train_acc.txt'), 'a')

    print("Start Training DenseNet121")
    ...
    for epoch in range(pre_epoch, EPOCH):
        ...
        if (epoch+1) % 1 == 0:
            ...
            f_acc1 = open(os.path.join(opt.train_result, 'DenseNet121', 'test_acc.txt'), 'a')
            f_loss1 = open(os.path.join(opt.train_result, 'DenseNet121', 'test_loss.txt'), 'a')
            with torch.no_grad():
                correct = 0
                total = 0
                labels_list = []
                ...
                if acc >= best_acc:
                    ...
                    torch.save(net.state_dict(), os.path.join(opt.train_result, 'DenseNet121', 'net_best.pth'))
    ...
    torch.save(net.state_dict(), os.path.join(opt.train_result, 'DenseNet121', 'net_latest.pth'))
