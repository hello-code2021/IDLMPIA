...
def main():
    ...
    print(labels)
    # create model
    model = ResNeXt50(num_classes=NUM_CLASS).to(device)
    # load model weights
    model.load_state_dict(torch.load(os.path.join(opt.model_weight_path,'ResNeXt50', 'net_best.pth'), map_location=device))
    model.eval()

    print('start to test')
    f_acc1 = open(os.path.join(opt.test_result_dir, 'ResNeXt50', 'test_acc.txt'), 'a')
    ...
    with torch.no_grad():
        ...
        # save the file.
        save_file(os.path.join(opt.test_result_dir, 'ResNeXt50', 'con_predicted.txt'), predited_list)
        save_file(os.path.join(opt.test_result_dir, 'ResNeXt50', 'con_labels.txt'), labels_list)
