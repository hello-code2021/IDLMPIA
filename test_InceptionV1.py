...
def main():
    ...
    print(labels)
    # create model
    model = InceptionV1(num_classes=NUM_CLASS).to(device)
    # load model weights

    model.load_state_dict(torch.load(os.path.join(opt.model_weight_path,'InceptionV1', 'net_best.pth'), map_location=device))

    model.eval()

    print('start to test')
    f_acc1 = open(os.path.join(opt.test_result_dir, 'InceptionV1', 'test_acc.txt'), 'a')
    ...
    with torch.no_grad():
        ...
        save_file(os.path.join(opt.test_result_dir, 'InceptionV1', 'con_predicted.txt'), predited_list)
        save_file(os.path.join(opt.test_result_dir, 'InceptionV1', 'con_labels.txt'), labels_list)
